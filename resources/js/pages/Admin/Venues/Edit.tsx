...
const initPrices = (raw: unknown): PriceCategories => {
    if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
        const r = raw as Record<string, unknown>;
        return {
            entree: Array.isArray(r.entree) ? r.entree : defaultPrices.entree,
            abonnementen: Array.isArray(r.abonnementen) ? r.abonnementen : [],
            groepen: Array.isArray(r.groepen) ? r.groepen : [],
            parkeren: Array.isArray(r.parkeren) ? r.parkeren : [],
            horeca: Array.isArray(r.horeca) ? r.horeca : [],
            overig: Array.isArray(r.overig) ? r.overig : [],
        };
    }
...
export default function VenuesEdit({ venue, types }: Props) {
    ...
    const venueData = venue as unknown as Record<string, unknown>;

    const { data, setData, patch, processing, errors } = useForm<FormData>({
        name: venue.name,
        type: venue.type,
        description: venue.description ?? '',
        opening_hours: initOpeningHours(venueData.opening_hours),
        prices: initPrices(venueData.prices),
        highlights: (venueData.highlights as string) ?? '',
        accessibility_transport: Array.isArray(venueData.accessibility_transport)
            ? (venueData.accessibility_transport as TransportEntry[])
            : [],
        accessibility_facilities: Array.isArray(venueData.accessibility_facilities)
            ? (venueData.accessibility_facilities as FacilityEntry[])
            : [],
        city: venue.city ?? '',
        country: venue.country ?? 'Nederland',
        ...
        <AdminLayout
            header={
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-semibold text-white">Bewerk: {venue.name}</h2>
                </div>
            }
        >
            <Head title={`Bewerk ${venue.name}`} />
