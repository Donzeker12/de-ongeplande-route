    ...
    const uploadFiles = async (files: File[]) => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const label = files.length > 1
                ? `Video ${i + 1} van ${files.length} — ${file.name}`
                : file.name;
        ...
        if (failed > 0) {
            setUploadError(`${failed} video('s) konden niet worden geüpload. Controleer het bestandstype en de grootte.`);
        }
        router.reload({ only: ['media'] });
    };
    ...
    const compressVideo = async (item: MediaItem) => {
        ...
        try {
            ...
            const msg = (err as { response?: { data?: { error?: string }; status?: number } })?.response?.data?.error
                ?? `Fout ${(err as { response?: { status?: number } })?.response?.status ?? ''}: kon video niet comprimeren.`;
            setActionError(msg);
                ...
                <div className="flex-1 p-4 sm:p-6 lg:p-8 min-w-0">
                    ...
                    {uploadProgress !== null && (
                        <div className="mb-5 p-4 bg-[#16181f] border border-gray-700 rounded-xl">
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className="text-sm font-semibold text-sky-400 shrink-0">{uploadProgress}%</span>
                            </div>
                    ...
                    {actionError && (
                        <div className="mb-5 flex items-center gap-3 p-3.5 bg-orange-500/10 border border-orange-500/20 rounded-xl text-orange-400 text-sm">
                            ...
                            Comprimeren mislukt: {actionError}
                        </div>
                    )}
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2">
                            ...
                            {media.map((item) => (
                                ...
                                >
                                    ...
                                    {/* Compressing overlay */}
                                    {(compressingId === item.id || item.processing) && (
                                        <div className="absolute inset-0 bg-black/70 backdrop-blur-sm flex flex-col items-center justify-center gap-2 z-10">
                                            ...
                                            <span className="text-white text-xs font-medium">Comprimeren...</span>
                                        </div>
                                    )}
                                    {/* Hover actions */}
